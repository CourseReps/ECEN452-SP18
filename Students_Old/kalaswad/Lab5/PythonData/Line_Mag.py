...
with open('S21_Line_dB.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        #items in '' below need to exactly match the entry in the first row of the columns in the .csv file
        #edit/add additional lines as needed for each column of data
        x.append(float(row['Freq(Hz)']))
        y1.append(float(row['Line S21 Mag']))
        y2.append(float(row['Line S21 Mag Sim']))
        y3.append(float(row['Thru S21 Mag']))
        y4.append(float(row['Thru S21 Mag Sim']))
...
ax1.plot(x, y2, 'g', label="Thru S21 Meas")
...
plt.title('S21 Calibration Standard Magnitude') #add plot title
