...
with open('PINSeriesSwitch-S11_S21_dB.csv') as csvfile:
	...
	for row in reader:
		...
		if row['Freq [GHz]']!="END":
			...
			y1.append(float(row['dB(S21_ON)']))
			y2.append(float(row['dB(S21_OFF)']))
...
##Read .csv data file
#replace quoted text below with filepath to your .csv file
with open('S21_ON_dB.csv') as csvfile:
	reader = csv.DictReader(csvfile)
	for row in reader:
        #items in '' below need to exactly match the entry in the first row of the columns in the .csv file
        #edit/add additional lines as needed for each column of data
...
##Read .csv data file
#replace quoted text below with filepath to your .csv file
with open('S21_OFF_dB.csv') as csvfile:
	reader = csv.DictReader(csvfile)
	for row in reader:
        #items in '' below need to exactly match the entry in the first row of the columns in the .csv file
        #edit/add additional lines as needed for each column of data
...
plt.title('PIN DIODE S21') #add plot title
plt.xlabel('Frequency [GHz]') #add x-axis title

ax1.set_ylabel('S21 Magnitude [dB]') #add y-axis title
