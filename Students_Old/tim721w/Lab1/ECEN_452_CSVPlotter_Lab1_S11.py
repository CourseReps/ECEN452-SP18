...
with open('ECEN_452_PlottingTestData_Lab1_S11.csv') as csvfile:
...
ax1.plot(x, y1, '-b', label="S11_Z0lver") #plot y1 vs. x, solid-blue, add lable for legend
ax1.plot(x, y2, '-r', label="S11_HFSS") #plot y2 vs. x, solid-red, add lable for legend
ax1.plot(x, y3, '--r', label="S11_Analytical") #plot y3 vs. x, dashed-red, add lable for legend
ax1.set_xlim(min(x), max(x)) #set x-axis limits
ax1.set_ylim(-18, -8) #set x-axis limits
...
plt.title('S11 Comparison Plot') #add plot title
