...
with open('Air.csv') as csvfile:
    ...
    for row in reader:
        ...
        y1.append(float(row['Air']))
        y2.append(float(row['Cardboard']))
        y3.append(float(row['Plexiglass']))

##Plotting
plt.figure(3) #initialize plot1
ax1 = plt.subplot(111) #create axes handle for plot1
ax1.plot(x, y1, 'b', label="Air") #plot y1 vs. x, solid-blue, add lable for legend
ax1.plot(x, y2, 'g', label="Cardboard")
ax1.plot(x, y3, 'r', label="Plexiglass")
...
plt.title('Free Space Measurements of Materials with GRL Calibration') #add plot title
