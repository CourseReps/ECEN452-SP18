...
with open('ECEN_452_PlottingTestData_Lab1_S21.csv') as csvfile:
...
ax1.plot(x, y1, '-b', label="S21_Z0lver") #plot y1 vs. x, solid-blue, add lable for legend
ax1.plot(x, y2, '-r', label="S21_HFSS") #plot y2 vs. x, solid-red, add lable for legend
ax1.plot(x, y3, '--r', label="S21_Analytical") #plot y3 vs. x, dashed-red, add lable for legend
ax1.set_xlim(min(x), max(x)) #set x-axis limits
ax1.set_ylim(-2, 0) #set x-axis limits
...
plt.title('S21 Comparison Plot') #add plot title
