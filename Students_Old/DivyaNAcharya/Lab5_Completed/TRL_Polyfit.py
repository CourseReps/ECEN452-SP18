...
phase = 89.7 #S21 phase of deembedded Line at center frequency
line_length = 13.8e-3 #additional Line length (physical length)
...
with open('Capacitance.csv') as csvfile:
...
C = -10e13/(2*np.pi*f*1e9*X) #convert reactance to capacitance
...
Capacitance_HFSS, = ax1.plot(f, C, '-b') #plot y1 vs. x, solid-blue, add lable for legend
Capacitance_Polynomial, = ax1.plot(f, p(f), '-r') #plot y2 vs. x, solid-red, add lable for legend
...
plt.legend([Capacitance_HFSS,Capacitance_Polynomial],['HFSS','Polynomial'],loc=4)
