...
with open('Milled_Hybrid_S11.csv') as csvfile:
    ...
    for row in reader:
        ...
        y1.append(float(row['S21-S31']))
        y2.append(float(row['S21-S31_HFSS']))

##Plotting
plt.figure(3) #initialize plot1
ax1 = plt.subplot(111) #create axes handle for plot1
ax1.plot(x, y1, 'b', label="S21-S31 Meas") #plot y1 vs. x, solid-blue, add lable for legend
ax1.plot(x, y2, 'g', label="S21-S31 Sim")
...
plt.title('Hybrid Coupler Phase Difference') #add plot title
