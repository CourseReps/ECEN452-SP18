...
if ser.isOpen():
    ...
    while True:
        ...
        set_volt = float(cmd_in)   # parse the string as binary
        if set_volt <= vcc:
            TX_bit = set_volt/vcc * 0xFFFF   # DAC conversion formula; 16 bit DAC resolution
            TX_cmd = DAC_cmd(CMD_WU_all, DAC_G, int(TX_bit))    # format data
