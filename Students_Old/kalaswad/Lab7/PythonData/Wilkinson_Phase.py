...
with open('Milled_Wilkinson_S11.csv') as csvfile:
    ...
    for row in reader:
        ...
        y1.append(float(row['S11 Phase(deg)']))
        y2.append(float(row['S21 Phase(deg)']))
        y3.append(float(row['S22 Phase(deg)']))
        y4.append(float(row['S31 Phase(deg)']))
        y5.append(float(row['S32 Phase(deg)']))
        y6.append(float(row['S33 Phase(deg)']))
        
with open('Lab7_Phase.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        #items in '' below need to exactly match the entry in the first row of the columns in the .csv file
        #edit/add additional lines as needed for each column of data
        #x.append(float(row['Freq(Hz)']))
        y7.append(float(row['S11 Phase(deg)']))
        y8.append(float(row['S21 Phase(deg)']))
        y9.append(float(row['S22 Phase(deg)']))
        y10.append(float(row['S31 Phase(deg)']))
        y11.append(float(row['S32 Phase(deg)']))
        y12.append(float(row['S33 Phase(deg)']))
...
plt.title('Wilkinson Power Divider Phase') #add plot title
